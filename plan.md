...
### 驗收標準
- [x] 重構前後產生的三份 .docx 內容一致（可用文字萃取後 diff）
- [x] `EmissionController.cs` 行數顯著下降
- [x] 修改一個共用欄位只需要改一個地方
...
| | 重構前 | 重構後 |
|---|---|---|
| `Controllers/EmissionController.cs` | 2,144 行 | **149 行**（−93%） |
| 三個報告書 action | 各約 450 行 | 各 **1 行**（委派給 builder） |
| `UpdateReplacePattern` 呼叫次數 | 290 | **86** |
| 新增 | — | `Services/GhgReportBuilder.cs`（共用流程＋原本的 DocX 輔助方法整批搬移）、`Services/GhgReportSpec.cs`（各標準差異描述） |

做法上先量測了三個方法到底差在哪，再決定怎麼切：**環境部與資策會兩份有 433 行中的 427 行完全相同（98.6%）**，實際差異只有範本檔名、多一張「公司組織圖」、以及資策會沒寫「盤查年」；ISO 則與環境部共用 470 行中的 346 行。所以差異全部集中成 `GhgReportSpec` 的三個 static 實例（範本檔／載入策略／類別小計來源／敘述樣式／圖片清單／四個 bool），並排看就是全部差異；共用流程只有一份。

**刻意不統一的地方**（統一會改變輸出或埋下地雷，都寫在程式碼註解裡）：
- 類別小計來源：ISO 即時把 `Device.Emissions` 加總，環境部／資策會讀 `Area.Scope1/Scope2` 等已存欄位。目前數值相同，但 `Area` 沒有類別三~六的欄位（ISO 需要），而且 `Device.Emissions` 累加所有氣體、`Area.Scope1` 只累加七種已知氣體，哪天多一種氣體兩者就會分岔。
- 排放源排序：ISO 原本那個「用 `device.Name` 去查以 Scope 當鍵的對照表」的排序其實完全不生效（每筆都落到 `int.MaxValue`），等於維持 EF 回傳順序；環境部／資策會是明確 `OrderBy(Scope).ThenBy(EmissionPattern)`。這個順序會影響所有表格與敘述，所以兩種載入策略都原樣保留，沒有「順手修正」。
- 範本用字：當年度是「占比」、基準年是「佔比」，兩份範本真的用不同字，統一寫法會有六格對不到標記而變空白。`N3`／`N4` 的不對稱（只有「總排放當量」與「基準年總排放當量」用 N3）同理保留。
- 「製程」段落結尾不補「，」（固定／移動／逸散都有補）、以及「類別一NF3占比」讀的是 `percentage2_NF3`：兩者三份報告書行為一致，屬既有行為，這次不動，避免混進輸出差異。

**驗證方式**（重構前先建立黃金檔）：先把測試資料補成「不退化」的狀態（原本 12 個排放源只有 2 個有活動數據、100% 集中在類別一，占比全是 0/100 這種比對不出差異的值）——補上類別二的外購電力用電、新增一個「製程」排放源、再用複製廠區功能造出 115 年度非基準年廠區，讓五種排放型態、兩個類別、基準年比較都有非整數的實際值。然後對「3 個廠區 × 3 份報告書」擷取黃金檔，比對兩個層次：
1. **替換字典**（key→value 排序後逐行比對，比只比 .docx 更敏感，能直接指出是哪個 key 變了）
2. **.docx 渲染文字**（解壓 `word/document.xml` 與頁首頁尾，每段落／表格儲存格一行）

結果：**原本會成功的 8 份報告書，渲染文字全部逐字相同**；替換字典 8 份中 6 份完全相同，另 2 份只差一個已預期的 `廠區名稱`（空字串，環境部／資策會原本在廠區名稱空白時不寫這個鍵，現在一律寫；因為找不到鍵時替換函式回傳空字串，渲染結果完全一樣，已由 .docx 比對證明）。

黃金檔比對走不到的兩條路徑另外補驗證：
- **自動產生的排放源敘述**（`Company.Scope1Information` 沒填時才會走到，測試公司剛好填了）：新增 `GhgReportNarrativeEquivalenceTests`，把重構前那 104 行原始寫法原封不動抄進測試，窮舉四種排放型態有／無的 16 種組合、每段 1~4 筆、同名去重等情境，逐一比對新舊輸出完全相同。
- **插圖路徑**（測試資料原本沒有任何圖片）：實際放圖驗過一次——ISO 嵌 2 張、環境部 2 張、資策會 3 張（多一張公司組織圖且順序在中間），與各標準原本的圖片清單和順序一致；驗完已把測試圖片與欄位還原。

**順手修掉的既有 bug（有輸出變化，刻意分開記錄）**：ISO 版報告書原本有 12 處占比是直接除以總排放量而沒有防零（`類別一~六占比`、`基準年類別一~六佔比`）。廠區完全沒有排放源時會 `DivideByZeroException` → HTTP 500 下載不了報告書；這個類別本來就有一個為此寫的 `PercentText` 防零輔助方法，但**一個呼叫點都沒有**（純死碼），環境部／資策會則是因為讀已存欄位而剛好沒事。重構後統一走 `PercentText`：實測同一個空廠區的 ISO 報告書從 500 變成正常產出（其餘 8 份輸出不變）。

測試從 73 個增加到 **88 個**（新增 12 個 spec/敘述測試 + 3 個差異等價測試），全數通過。
...
**已完成（階段 3：角色管理介面／矩陣勾選）**：
- 新增 `Services/PermissionMatrixService.cs`：把「畫面 × 動作」的權限目錄組成矩陣（列＝畫面、欄＝動作），並把「勾選狀態的比對與存檔」獨立成 `SaveRoleMatrixAsync`／`SaveUserMatrixAsync`（純資料存取邏輯，不依賴 MVC，方便單元測試）。
- `RolePermissionsController` 新增 `Matrix(string? roleId)`（GET／POST），畫面（`Views/RolePermissions/Matrix.cshtml`）用角色頁籤切換、每個畫面一列、每個動作一欄的勾選表格取代原本「新增一筆角色權限」要選角色下拉、選權限下拉、送出，76 個權限要重複 76 次的流程；一次送出即可把整個角色的權限設定成表格上勾選的狀態（多的刪、少的補，交易內完成）。額外加上「整列／整欄快速勾選」按鈕（純前端 JS，方便一次授權整個畫面或整個動作）。原本的 `Index`／`Create`／`Edit`／`Delete` 一次一筆 CRUD 保留不動（互有連結可切換），沒有拿掉任何既有功能。
- `Admin` 角色不會出現在角色選單中——`PermissionFilterAttribute` 對 `Admin` 一律放行，在這裡設定完全沒有效果，列出來只會讓人誤以為勾選有作用。

**已完成（階段 4：使用者個別權限）**：
- 新增 `Controllers/UserPermissionsController.cs` ＋ `Views/UserPermissions/Matrix.cshtml`：`UserPermission` 表原本存在但完全沒有維護介面（資料表永遠是空的，見上面階段 1／2 的紀錄），現在有了一個使用者一個畫面的勾選介面。
- 由於現有資料結構的限制（`HasPermissionAsync` 對角色權限與個別權限是「取聯集」，沒有「個別收回角色已授予的權限」的表達方式），矩陣把「已經由角色授予」的格子畫成勾選且鎖住（disabled、無 `name` 屬性，瀏覽器送出表單時完全不會帶到這個 Id），並標註「已由角色授予，無法在此收回」；管理員只能在角色範圍之外「額外授予」個別權限，要調整角色本身的權限請到角色權限設定。
- `SaveUserMatrixAsync` 的比對範圍**排除**角色已授予的權限 Id（不論表單有沒有帶到都不動它），避免鎖住的格子被誤判成「使用者想取消勾選」而刪掉使用者原本就有的個別授權——這是這個功能裡最容易踩到的地雷，寫了專門的回歸測試釘住。
- Admin 角色成員不會出現在使用者選單中；即使直接組 POST 夾帶 Admin 使用者的 Id，伺服器端也會重新檢查並回傳 403。
- `Data/ApplicationDbContext.cs` 的 `AuditedEntityTypes` 加入 `UserPermission`（原本只有 `RolePermission`），矩陣送出的整批新增／刪除因此會自動進稽核軌跡，不需要額外寫程式；`AuditLogsController` 的實體篩選下拉也同步加上 `UserPermission`。
- 新增 15 筆單元測試（`PermissionMatrixServiceTests`），涵蓋矩陣建立（含「畫面沒定義這個動作」顯示成空格）、儲存時的新增／刪除／不影響其他角色或使用者、不存在的權限 Id 被忽略、以及上述「角色已授予的格子不會被誤刪」的地雷測試；全專案測試共 104 個全數通過。
- 實機驗證：以 Admin 身分對 `Guest` 角色勾選「排放係數設定」全部 5 個動作並儲存，資料庫確認剛好新增這 5 筆、稽核軌跡出現 5 筆對應的 `Create RolePermission` 紀錄；對已有 `PowerUser` 角色（僅授權「排放係數設定-檢視」）的測試帳號額外勾選「明細」個別權限，資料庫確認只新增 1 筆 `UserPermission`、角色本身的授權沒有被複製進來；**改用該帳號實際登入**，確認 `/Materials`（角色授權）與 `/Materials/Details/{id}`（新授予的個別權限）皆可存取、`/Materials/Edit/{id}`（未授權）被擋下，證明矩陣寫入的資料真的被 `PermissionFilterAttribute` 讀取生效，不只是存進一張沒人讀的表；測試後已還原 Guest 角色與該帳號的個別權限，PowerUser 角色本身的授權不受影響。

**併發保護（實機測試時發現並修正）**：矩陣是「整批覆蓋」語意——送出當下的勾選內容會直接取代資料庫裡的授權集合。如果兩個管理員前後編輯同一個角色／使用者，後送出的表單會拿「自己載入畫面時」的舊資料去覆蓋，悄悄蓋掉對方存的異動。修法：`PermissionMatrixViewModel.Baseline` 記錄「建立矩陣當下」的授權集合指紋，存進表單的隱藏欄位；`SaveRoleMatrixAsync`／`SaveUserMatrixAsync` 存檔前重新比對資料庫目前的實際狀態，不一致就拒絕存檔並提示「請重新整理後再試一次」，而不是悄悄覆蓋。過程中在瀏覽器實測時另外抓到一個真正的 bug：ASP.NET Core 的表單繫結會把「值是空字串」的欄位直接繫結成 `null`（視同完全沒有這個欄位），而角色／使用者「目前完全沒有授權」時 Baseline 剛好就是空字串——會被誤判成「呼叫端沒有提供 baseline，不用檢查」，讓併發保護在這個情境下整個失效。修成 `EncodedBaseline`／`DecodeExpectedBaseline`：隱藏欄位一律加上固定前綴，確保送出的值永遠不是空字串。另外對「雙擊送出、兩個分頁同時存檔」造成的唯一索引衝突加上一次重試（清空變更追蹤器、用資料庫最新狀態重算一次差異再存一次），避免直接跳出未處理例外。新增 6 筆對應的單元測試（含完整模擬「View 編碼、Controller 解碼」路徑的迴歸測試），並在瀏覽器裡實際重現這個併發情境（用 SQL 直接插入模擬另一個管理員搶先存檔）驗證衝突會被正確攔截、對方的異動不會被蓋掉、重新整理後可以正常恢復存檔；全專案測試共 114 個全數通過。

**尚未開始**：`Areas`／`Devices`／`Emission` 三個核心流程 Controller 尚未掛上 `PermissionFilterAttribute`（風險較高，待階段 1 的四個低風險畫面確認穩定後再擴及）；`Companies`／`User`／`Functions`／`FunctionActions`／`Permissions`／`RolePermissions`／`UserPermissions`／`Feedbacks` 目前仍是寫死的 `[Authorize(Roles="Admin")]`，尚未接上權限表。
...
## 第四階段：長期維護性
8. **[A3][A4][A5]** 效能優化 ✅ 已完成
9. **[A8] EmissionController 重構** ✅ 已完成 —— 2,144 行降到 149 行；重構前先建立「替換字典＋.docx 渲染文字」雙層黃金檔，證明原本會成功的 8 份報告書輸出逐字不變
